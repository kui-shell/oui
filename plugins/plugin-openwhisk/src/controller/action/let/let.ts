...
const fetchRemote = (location: string, mimeType: string): Promise<Remote> =>
    ...
    try {
      ...
      lstat(location, err => {
        if (err) {
          ...
          reject(
            new Error(
              `I think you asked to create a zip action, but the specified zip file does not exist: ${location}`
            )
          )
        } else {
...
export default async (commandTree: Commands.Registrar) => {
  ...
  const doCreate = (args: Commands.Arguments): Promise<Commands.Response> => {
     ...
     *
     */
    const packageAutoCreate = (name: string) => (err1: StatusCodeError) => {
      if (err1.statusCode === 404 && !execOptions.noRetry) {
        ...
        if (packageName) {
          return REPL.qexec(`wsk package update "${packageName}"`)
            .then(() => doCreate(Object.assign({}, args, { noRetry: true })))
            .catch(() => {
              // package auto-create failed, rethrow the first error
              throw err1
            })
        }
      }
    ...
    if (actionMatch && !isSequenceMatch) {
      ...
      if (sequenceMatch) {
        ...
        if (components.length >= 2) {
          ...
          const actionFromFileMatch =
            command.match(patterns.action.expr.fromFileWithExtension) || command.match(patterns.action.expr.fromFile)
