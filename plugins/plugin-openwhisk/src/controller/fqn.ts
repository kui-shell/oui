 ...
 *
 */
export function fqn(resource: { metadata: { name: string; namespace?: string } }): string {
  return `/${resource.metadata.namespace ? resource.metadata.namespace + '/' : ''}${resource.metadata.name}`
}
