...
import { sort, sortActivations, startTimeSorter, countSorter } from '../lib/sorting'
import { GroupData, TimelineData, groupByAction } from '../lib/grouping'
...
class Occupancy {
  ...
  constructor(width: number, height: number, nCells: number, grid: HTMLElement, gridGrid: HTMLElement, tab: Tab) {
    this.tab = tab
    this.width = width
    this.height = height
...
const _drawGrid = (
  ...
  const { groups, timeline } = groupData
  ...
  groups.forEach(group => {
    ...
    if (!redraw) {
      ...
      const width = L
      const height = L
      const grid = new Occupancy(width, height, group.activations.length, gridDom, gridGrid, tab)
      group.x = 0
      group.y = 0
      group.width = L
      group.height = L
 ...
 *
 */
async function content2(tab: Tab, data: ActivationData<GridOptions>, content: HTMLElement) {
  const { parsedOptions, activations } = data.content
  const options = parsedOptions

  content.classList.add(css.content)
  content.classList.add(css.useDarkTooltips)

  _drawGrid(tab, options, content, groupByAction(activations, options), undefined, undefined)
  ...
  const asTimeline: UI.Mode = {
    ...
    flush: 'right',
    actAsButton: true,
    direct: () => tab.REPL.pexec(`grid ${optionsToString(options)} -t`)
  }
  const asGrid: UI.Mode = {
    ...
    flush: 'right',
    actAsButton: true,
    direct: () => tab.REPL.pexec(`grid ${optionsToString(options, ['timeline', 't'])}`)
  } */

  return content
}

async function content(tab: Tab, data: ActivationData<GridOptions>) {
  injectContent()

  // to allow injectContent to finish
  const content = document.createElement('div')
  setTimeout(() => content2(tab, data, content), 0)
  return content
}
