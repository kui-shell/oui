...
import { sort, statDataSorter } from '../lib/sorting'
import { OutlierReason, groupByAction } from '../lib/grouping'
...
async function content2(tab: Tab, data: ActivationData<TableOptions>, content: HTMLElement) {
  const { parsedOptions, eventBus, activations } = data.content
  ...
  } else if (parsedOptions.ww) {
    // user asked for the action name column to be wider
    content.classList.add('extra-wide-label')
  }
  // add time range to the sidecar header
  const groupData = groupByAction(activations, Object.assign({ groupBySuccess: true }, parsedOptions))

  const sorter = statDataSorter(defaultTop)
  const sortDir = +1
  ...
  interface DrawArgs {
    ...
    top: string | number
    redraw?: boolean
    showOutliers?: true | number
  }
  const draw = ({ bottom: stat25, top: stat75, redraw = false, showOutliers = undefined }: DrawArgs) => {
  ...
  eventBus.on('/summary/range/outliers/toggle', ({ showOutliers }: { showOutliers: boolean }) => {
    content.classList.toggle('show-outliers')
    const args: DrawArgs = Object.assign({}, currentRange, { redraw: true, showOutliers })
    draw(args)
  })
...
async function content(tab: Tab, data: ActivationData<TableOptions>) {
  injectContent()
