...
import { Row, Table, Arguments, ParsedOptions, Registrar } from '@kui-shell/core'
...
interface ListOptions extends ParsedOptions {
  name?: string
  count?: number
  limit?: number
  skip?: number
...
export default async (commandTree: Registrar) => {
  commandTree.listen(
    `/wsk/app/list`,
    ({ argvNoOptions, parsedOptions, execOptions, REPL }: Arguments<ListOptions>) => {
        ...
        if (actions.body) {
          const apps = actions.body.filter(astUtil.isAnApp).map(app =>
            Object.assign({}, app, {
              ...
              prettyType,
              prettyKind,
              onclick: `wsk app get "/${app.metadata.namespace}/${app.metadata.name}"`
            })
          )
